...
void merge(TermPtr t1, TermPtr t2){
    vector<TermPtr> v1;
    vector<TermPtr> v2;
    for(const auto& it : solution[findT[t1]]){
    	for(auto& it2 : useT[it])
        	v1.push_back(it2);
    }

    for(const auto& it : solution[findT[t2]]){
        for(auto& it2 : useT[it])
        	v2.push_back(it2);
    }
    unionT(t1,t2);
...
void unionT(TermPtr t1, TermPtr t2){
    for(auto& it : solution[findT[t2]]){
    	solution[findT[t1]].push_back(it);
    	if(equal(it, findT[t2]))
    		continue;
    	findT[it] = findT[t1];
    }
    solution.erase(findT[t2]);
    findT[t2] = findT[t1];
}
